...
avg10DaysVolume	number($double)

Average 10 day volume
avg1YearVolume	number($double)
...
Enum:
Array [ 8 ]
divPayAmount	number($double)
example: 0.22

Dividend Pay Amount
divPayDate	string($date-time)
example: 2021-05-13T00:00:00Z
pattern: yyyy-MM-dd'T'HH:mm:ssZ

Dividend pay date in yyyy-mm-ddThh:mm:ssZ
divYield	number($double)
example: 0.7
...
FSI Desc
htbQuantity	integer($int32)
example: 100

Hard to borrow quantity.
htbRate	number($double)
example: 4.5
...
regularMarketLastPrice	number($double)
example: 124.85

Regular market last price
regularMarketLastSize	integer($int32)
example: 51771

Regular market last size
regularMarketNetChange	number($double)
example: -1.42
...
regularMarketTradeTime	integer($int64)
example: 1621368000400
...
Mark Price percent change
moneyIntrinsicValue	number($double)
example: -947.96
...
Exchange Name
exerciseType	ExerciseTypestring

option contract exercise type America or European
Enum:
[ A, E ]
expirationDay	integer($int32)
example: 20
maximum: 31
minimum: 1
...
Expiration Month
expirationType	ExpirationTypestring
...
Previous day's closing price
futurePercentChange	number($double)
example: -0.0756
...
Active symbol
futureExpirationDate	number($int64)
...
Future is active
futureMultiplier	number($double)
example: 50
...
Price format
futureSettlementPrice	number($double)
example: 4123
...
Enum:
[ BOND, EQUITY, FOREX, FUTURE, FUTURE_OPTION, INDEX, MUTUAL_FUND, OPTION ]
assetSubType	MutualFundAssetSubTypestring
nullable: true
...
delayed	boolean
description	string
exchangeName	stringEnum:
[ IND, ASE, NYS, NAS, NAP, PAC, OPR, BATS ]
