...
class LightweightChartWidget(QtWebEngineWidgets.QWebEngineView):
    ...
    def _init_chart(self) -> None:
        ...
        html = (
            ...
            " if(!bar || bar.time===undefined|| bar.time===null){return null;}"
            ...
            " const filtered=data.filter(b=>b && b.time!==undefined && b.time!==null);"
            " candleSeries.setData(filtered);"
            " const vol=filtered.map(withVolumeColor).filter(v=>v!==null);"
            " volumeSeries.setData(vol);"
            "};"
            "window.lwc_update=function(bar){"
            " if(!bar || bar.time===undefined || bar.time===null){return;}"
            " candleSeries.update(bar);"
            " const vb=withVolumeColor(bar);"
