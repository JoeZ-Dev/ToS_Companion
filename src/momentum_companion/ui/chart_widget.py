...
class LightweightChartWidget(QtWebEngineWidgets.QWebEngineView):
    ...
    def _init_chart(self) -> None:
        ...
        html = (
            ...
            "#tooltip{position:absolute;display:none;min-width:120px;z-index:15;pointer-events:none;"
            "background:transparent;border:none;box-shadow:none;color:#c8d1da;font:12px 'Segoe UI',sans-serif;}"
            ...
            " const items=["
            "  {name:'VOLUME', label:'Volume', color:'#888888'},"
            "  {name:'VWAP', label:'VWAP', color:lineStyles.VWAP.color},"
            "  {name:'EMA9', label:'EMA9', color:lineStyles.EMA9.color},"
            "  {name:'EMA20', label:'EMA20', color:lineStyles.EMA20.color},"
            " ];"
            " menu.innerHTML='';"
            " items.forEach(item=>{"
            ...
            " toolTip.innerHTML=`"
            "  <div class='legend-row'><span>Time</span><span>${utcToET(ts)}</span></div>"
            "  ${priceRow}"
            "  ${volRow}"
            " `;"
